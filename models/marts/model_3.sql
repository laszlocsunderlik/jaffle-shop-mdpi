...
ranked_customers as (
    select 
        mt.*,
        row_number() over (partition by month_of_the_year order by total_invoiced desc) as rank
...
join {{ source('raw_jaffle_shop_data', 'raw_customers') }} c on rc.customer = c.id
where rc.rank <= 3
